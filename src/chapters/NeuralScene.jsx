...
// Interactive 3D constellation, carried over from the previous site and
// re-coloured from CSS tokens (--orb-*) so it follows the light/dark theme. Nodes fire in sequence like a pipeline
...
export default function NeuralScene() {
  ...
  useEffect(() => {
    ...
    const baseMat = () => new THREE.MeshBasicMaterial({ color: 0x3a4658, transparent: true, opacity: 0.9 });
    ...
    const lineMat = new THREE.LineBasicMaterial({ color: 0x8f88ff, transparent: true, opacity: 0.18 });
    const edges = [];
    nodes.forEach((a, i) => {
      const dists = nodes
    ...
    const trail = new THREE.Color();
    const pulse = new THREE.Color();
    const dim = new THREE.Color();

    // Pull colours from the theme tokens; re-run whenever data-theme flips.
    const applyTheme = () => {
      const cs = getComputedStyle(document.documentElement);
      const token = (name, fallback) => cs.getPropertyValue(name).trim() || fallback;
      pulse.set(token('--orb-pulse', '#c4ee2f'));
      trail.set(token('--orb-trail', '#8f88ff'));
      dim.set(token('--orb-node', '#3a4658'));
      lineMat.color.set(token('--orb-edge', '#8f88ff'));
      nodes.forEach((n) => n.material.color.copy(dim));
