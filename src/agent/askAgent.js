...
const ERRORS = {
  429: 'Rate limit reached. Give it a minute — or just email Rizky directly.',
  503: 'The agent is offline right now. Email is the fastest route.',
...
export default async function askAgent(messages) {
  ...
  try {
    ...
    if (!res.ok) {
      return { ok: false, error: ERRORS[res.status] || 'Something broke on my side. Email works while I fix it.' };
    }
